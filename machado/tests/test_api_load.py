...
class OrganismViewSetTest(BaseLoadViewSetTest):
    ...
    def test_destroy_success(self, mock_thread):
        ...
        self.assertEqual(mock_thread.call_args.kwargs['kwargs'], {'organism': 'Genus Species'})
...
class RelationsOntologyViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/relations_ontology', {'file': uploaded_file}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
        self.assertEqual(mock_thread.call_args.kwargs['args'], ("load_relations_ontology",))
        mock_file.assert_called_with('/tmp/ro.obo', 'wt')

    @patch('machado.api.views.load.Thread')
    def test_destroy_success(self, mock_thread):
        view = RelationsOntologyViewSet.as_view({'delete': 'destroy'})
        request = self.factory.delete('/api/load/relations_ontology', {'name': 'test_cv'})
        force_authenticate(request, user=self.user)
        response = view(request)
...
class PublicationViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/publication', {'file': uploaded_file, 'cpu': 4}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
...
class SequenceOntologyViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/sequence_ontology', {'file': uploaded_file}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
        self.assertEqual(mock_thread.call_args.kwargs['args'], ("load_sequence_ontology",))
...
class GeneOntologyViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/gene_ontology', {'file': uploaded_file}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
...
class FastaViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_no_file(self):
        view = FastaViewSet.as_view({'post': 'create'})
        request = self.factory.post('/api/load/fasta', {'organism': 'org', 'soterm': 'chr'}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_create_missing_params(self):
        ...
        # Missing organism
        request = self.factory.post('/api/load/fasta', {'file': uploaded_file, 'soterm': 'chr'}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        
        # Missing soterm
        request = self.factory.post('/api/load/fasta', {'file': uploaded_file, 'organism': 'org'}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class FeatureAnnotationViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/feature_annotation', data, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
        self.assertEqual(mock_thread.call_args.kwargs['args'], ("load_feature_annotation",))
    ...
    def test_create_missing_params(self):
        ...
        for data, missing in cases:
            if missing != 'file': data['file'] = f
            request = self.factory.post('/api/load/feature_annotation', data, format='multipart')
            force_authenticate(request, user=self.user)
            response = view(request)
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST, f"Failed for missing {missing}")
...
class FeatureSequenceViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/feature_sequence', data, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
        self.assertEqual(mock_thread.call_args.kwargs['args'], ("load_feature_sequence",))
...
class FeaturePublicationViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/feature_publication', {'file': uploaded_file}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
        self.assertEqual(mock_thread.call_args.kwargs['args'], ("load_feature_publication",))

    def test_create_no_file(self):
        view = FeaturePublicationViewSet.as_view({'post': 'create'})
        request = self.factory.post('/api/load/feature_publication', {}, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class FeatureDBxRefViewSetTest(BaseLoadViewSetTest):
    ...
    def test_create_success(self, mock_file, mock_thread):
        ...
        request = self.factory.post('/api/load/feature_dbxrefs', data, format='multipart')
        force_authenticate(request, user=self.user)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_thread.assert_called_once()
        self.assertEqual(mock_thread.call_args.kwargs['args'], ("load_feature_dbxrefs",))
