...
from machado.models import Cv, Cvterm, Cvtermprop, CvtermDbxref, Cvtermsynonym, CvtermRelationship, Db, Dbxref
...
class OntologyLoaderTest(TestCase):
    ...
    def test_store_type_def(self):
        ...
        # Check props
        self.assertTrue(Cvtermprop.objects.filter(cvterm=cvterm, type__name="comment", value="comment1").exists())
        self.assertTrue(Cvtermprop.objects.filter(cvterm=cvterm, type__name="is_class_level", value="1").exists())
        self.assertTrue(Cvtermprop.objects.filter(cvterm=cvterm, type__name="is_metadata_tag", value="1").exists())
        self.assertTrue(Cvtermprop.objects.filter(cvterm=cvterm, type__name="is_symmetric", value="1").exists())
        self.assertTrue(Cvtermprop.objects.filter(cvterm=cvterm, type__name="is_transitive", value="1").exists())
        
        # Check xref
        self.assertTrue(CvtermDbxref.objects.filter(cvterm=cvterm, dbxref__db__name="REF", dbxref__accession="123").exists())
    ...
    def test_store_term(self):
        ...
        # Check alt_id
        self.assertTrue(CvtermDbxref.objects.filter(cvterm=cvterm, dbxref__db__name="SO", dbxref__accession="0000704", is_for_definition=0).exists())
        
        # Check comment
        self.assertTrue(Cvtermprop.objects.filter(cvterm=cvterm, type__name="comment", value="a comment").exists())
        
        # Check xref
        self.assertTrue(CvtermDbxref.objects.filter(cvterm=cvterm, dbxref__db__name="XREF", dbxref__accession="789").exists())
        
        # Check synonym
        self.assertTrue(Cvtermsynonym.objects.filter(cvterm=cvterm, synonym="syn1").exists())
    ...
    def test_store_relationship(self):
        ...
        self.assertTrue(CvtermRelationship.objects.filter(subject=coding, object=gene, type=is_a).exists())
        self.assertTrue(CvtermRelationship.objects.filter(subject=coding, object=gene, type=part_of).exists())

    def test_process_cvterm_go_synonym(self):
        cvterm = Cvterm.objects.get(name="is_a") # reuse any
        self.loader.process_cvterm_go_synonym(cvterm, "\"assembly\" [GOC:123]", "EXACT_synonym")
        self.assertTrue(Cvtermsynonym.objects.filter(cvterm=cvterm, synonym="assembly", type__name="exact").exists())
    ...
    def test_process_cvterm_def_edge_cases(self):
        ...
        self.assertTrue(CvtermDbxref.objects.filter(cvterm=cvterm, dbxref__db__name="URL").exists())
