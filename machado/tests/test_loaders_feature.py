...
from machado.loaders.feature import FeatureLoaderBase, FeatureLoader, MultispeciesFeatureLoader
from machado.loaders.exceptions import ImportingError
from machado.models import Cv, Cvterm, Db, Dbxref, Organism, Feature, Pub, PubDbxref, Featureloc, FeatureRelationship, FeatureRelationshipprop, FeatureDbxref, Featureprop, FeatureCvterm
...
class FeatureLoaderTest(TestCase):
    ...
    def ensure_cvterm(self, name, cv):
        dbxref, created = Dbxref.objects.get_or_create(db=self.db_internal, accession=f"acc_{name}")
        return Cvterm.objects.get_or_create(
            name=name, cv=cv, is_obsolete=0,
            defaults={
                'dbxref': dbxref,
    ...
    def create_feat(self, uniquename, type_obj, dbxref=None, organism=None):
        if organism is None:
            organism = self.org
        if dbxref is None:
            dbxref = Dbxref.objects.create(db=self.db_internal, accession=f"dbx_{uniquename}")
        return Feature.objects.create(
            organism=organism, uniquename=uniquename, type=type_obj,
            dbxref=dbxref, is_analysis=False, is_obsolete=False,
            timeaccessioned=datetime.now(timezone.utc), timelastmodified=datetime.now(timezone.utc)
        )
    ...
    def test_loader_base_init_pub_dbxref_fail(self):
        ...
        with self.assertRaisesRegex(ImportingError, "10.1234/missing_pub not registered"):
    ...
    def test_store_tabix_GFF_feature_success(self, MockAttrLoader):
        ...
        mock_attrs.get_attributes.return_value = {"id": "FEAT_S", "name": "Name1"}
        db_fasta = Db.objects.get_or_create(name="FASTA_SOURCE")[0]
        dbxref_contig = Dbxref.objects.create(db=db_fasta, accession="CONTIG_S")
        self.create_feat("CONTIG_S", self.ensure_cvterm("chromosome", self.cv_seq), dbxref=dbxref_contig)
        loader.store_tabix_GFF_feature(tabix_feat, qtl=False)
        self.assertTrue(Feature.objects.filter(uniquename="FEAT_S", type__name="mRNA").exists())
        self.assertTrue(Feature.objects.filter(uniquename="FEAT_S", type__name="polypeptide").exists())
    ...
    def test_store_tabix_GFF_feature_strand_minus_and_phase_error(self, MockAttrLoader):
        ...
        self.create_feat("C_SM", self.ensure_cvterm("chromosome", self.cv_seq), dbxref=dbxref_c)
        self.ensure_cvterm("translation_of", self.cv_seq)
        loader.store_tabix_GFF_feature(tabix_feat, qtl=False)
    ...
    def test_store_tabix_GFF_feature_no_id(self, MockAttrLoader):
        ...
        db_fasta = Db.objects.get_or_create(name="FASTA_SOURCE")[0]
        dbxref_contig = Dbxref.objects.get_or_create(db=db_fasta, accession="CONTIG_NOID")[0]
        self.create_feat("CONTIG_NOID", self.ensure_cvterm("chromosome", self.cv_seq), dbxref=dbxref_contig)
        loader.store_tabix_GFF_feature(tabix_feat, qtl=False)
    ...
    def test_store_tabix_GFF_feature_qtl(self, MockAttrLoader):
        ...
        db_fasta = Db.objects.get_or_create(name="FASTA_SOURCE")[0]
        dbxref_contig = Dbxref.objects.get_or_create(db=db_fasta, accession="CONTIG_QTL")[0]
        self.create_feat("CONTIG_QTL", self.ensure_cvterm("chromosome", self.cv_seq), dbxref=dbxref_contig)
        loader.store_tabix_GFF_feature(tabix_feat, qtl=True)
        self.assertTrue(Feature.objects.filter(uniquename="QTL_F", type__name="QTL").exists())
    ...
    def test_store_tabix_GFF_feature_integrity_error(self, MockAttrLoader):
        ...
        # Now storing it again should trigger IntegrityError and then ImportingError
        with self.assertRaises(ImportingError):
             loader.store_tabix_GFF_feature(tabix_feat, qtl=False)
    ...
    def test_store_relationship(self):
        ...
        self.assertTrue(FeatureRelationship.objects.filter(subject=f1, object=f2).exists())
    ...
    def test_store_feature_dbxref(self):
        ...
        self.assertTrue(FeatureDbxref.objects.filter(feature=f1, dbxref__accession="ACC_DBX", dbxref__db__name="EXT").exists())
    ...
    def test_store_feature_groups(self):
        ...
        loader.store_feature_groups(["f_grp_1", "f_grp_2"], mRNA_type, self.org, soterm="mRNA_G")
        self.assertEqual(Featureprop.objects.filter(feature=f1, type=mRNA_type).count(), 1)
        self.assertEqual(Featureprop.objects.filter(feature=f2, type=mRNA_type).count(), 1)
    ...
    def test_store_feature_groups_acc_none(self):
        ...
        loader.store_feature_groups(["nonexistent_acc", None], unique_type, self.org, soterm="mRNA_U_N")
    ...
    def test_store_feature_groups_integrity_error(self):
        ...
        f2 = self.create_feat("f_grp_ie_2", mRNA_type)
        ...
        with self.assertRaises(ImportingError):
            loader.store_feature_groups(["f_grp_ie_1", "f_grp_ie_2"], mRNA_type, self.org, soterm="mRNA_G_IE")
    ...
    def test_multispecies_retrieve_feature_id(self):
        ...
        self.assertEqual(loader.retrieve_feature_id("UNIQUE_M_1", "protein_match"), f1.feature_id)
    ...
    def test_multispecies_retrieve_feature_id_fallbacks(self):
        ...
        self.assertEqual(loader.retrieve_feature_id("FEAT_F_2", "mRNA_F"), f2.feature_id)
        f3 = self.create_feat("FEAT_F_3", mRNA_type)
        f3.name = "DISPLAY_F_3"
        f3.save()
        self.assertEqual(loader.retrieve_feature_id("DISPLAY_F_3", "mRNA_F"), f3.feature_id)
        ...
        self.assertEqual(loader.retrieve_feature_id("DBACC_F_4", "mRNA_F"), f4.feature_id)
        ...
        self.assertEqual(loader.retrieve_feature_id("SECACC_F_5", "mRNA_F"), f5.feature_id)
    ...
    def test_multispecies_store_bio_searchio_hit(self):
        ...
        Cvterm.objects.get_or_create(name="term1", cv=self.cv_seq, dbxref=dbxref_go, is_obsolete=0, is_relationshiptype=0)
        ...
        self.assertTrue(FeatureDbxref.objects.filter(feature=feat, dbxref__accession="X1").exists())
    ...
    def test_multispecies_store_bio_searchio_hit_no_acc_and_go_fail(self):
        ...
        hit_mock = MagicMock()
        hit_mock.id = "HIT_X_1"
        if hasattr(hit_mock, 'accession'): del hit_mock.accession 
        hit_mock.dbxrefs = ["GO:9999", "OTHER:O1"]
        loader.store_bio_searchio_hit(hit_mock, target="Blast")
        self.assertIn("GO:9999", loader.ignored_goterms)
        feat = Feature.objects.get(uniquename="HIT_X_1")
        self.assertTrue(FeatureDbxref.objects.filter(feature=feat, dbxref__accession="O1").exists())

    def test_store_feature_publication(self):
        loader = FeatureLoader("GFF_PUB", "test.gff", self.org)
        f1 = self.create_feat("f_pub_1", self.cvterm_poly)
        db_doi = Db.objects.get_or_create(name="DOI")[0]
        dbxref_doi = Dbxref.objects.get_or_create(db=db_doi, accession="10.1234/test_pub_1")[0]
        null_cvterm = Cvterm.objects.get(name="null", cv__name="null")
        pub = Pub.objects.create(uniquename="pub_unique_1", type=null_cvterm, is_obsolete=False)
        PubDbxref.objects.create(pub=pub, dbxref=dbxref_doi, is_current=True)
        loader.store_feature_publication("f_pub_1", "polypeptide", "10.1234/test_pub_1")
        self.assertTrue(Feature.objects.get(uniquename="f_pub_1").FeaturePub_feature_Feature.exists())
    ...
    def test_store_tabix_VCF_feature_success(self, MockAttrLoader):
        ...
        db_fasta = Db.objects.get_or_create(name="FASTA_SOURCE")[0]
        dbxref_contig = Dbxref.objects.get_or_create(db=db_fasta, accession="CONTIG_VCF_S")[0]
        self.create_feat("CONTIG_VCF_S", self.ensure_cvterm("chromosome", self.cv_seq), dbxref=dbxref_contig)
        self.ensure_cvterm("SNP", self.cv_seq)
        Cvterm.objects.get_or_create(name="quality_value", cv=self.cv_seq, 
                                     defaults={'dbxref': Dbxref.objects.create(db=self.db_internal, accession="qual_acc_v_s"), 
                                               'is_obsolete': 0, 'is_relationshiptype': 0})
        loader.store_tabix_VCF_feature(tabix_feat)
        self.assertTrue(Feature.objects.filter(uniquename="VAR_S_1").exists())
        self.assertEqual(Featureloc.objects.filter(feature__uniquename="VAR_S_1").count(), 3)
    ...
    def test_store_tabix_VCF_feature_pub_and_loc_ie(self, MockAttrLoader):
        db_doi = Db.objects.get_or_create(name="DOI")[0]
        dbxref_doi = Dbxref.objects.get_or_create(db=db_doi, accession="10.1234/vcf_doi")[0]
        # Trigger initialization of null objects
        FeatureLoaderBase("VCF", "dummy.vcf")
        null_cvterm = Cvterm.objects.get(name="null", cv__name="null")
        pub = Pub.objects.create(uniquename="vcf_pub", type=null_cvterm, is_obsolete=False)
        PubDbxref.objects.create(pub=pub, dbxref=dbxref_doi, is_current=True)
        ...
        self.create_feat("C1", self.ensure_cvterm("chromosome", self.cv_seq), dbxref=dbxref_c1)
        self.ensure_cvterm("SNP", self.cv_seq)

        with patch('machado.models.FeaturePub.objects.get_or_create', side_effect=IntegrityError("Pub IE")):
            with self.assertRaises(ImportingError):
                loader.store_tabix_VCF_feature(tabix_feat)
        
        with patch('machado.models.Featureloc.objects.get_or_create', side_effect=IntegrityError("Loc IE")):
            with self.assertRaises(ImportingError):
                loader.store_tabix_VCF_feature(tabix_feat)
    ...
    def test_store_feature_pairs(self):
        ...
        loader.store_feature_pairs(["f_pair_1", "f_pair_2"], self.cvterm_poly, soterm="mRNA_P")
        self.assertTrue(FeatureRelationship.objects.filter(subject=f1, object=f2, type=self.cvterm_poly).exists())
    ...
    def test_store_feature_annotation(self, MockAttrLoader):
        ...
        f1 = self.create_feat("f_ann_1", mRNA_type)
        mock_attrs = MockAttrLoader.return_value
        mock_attrs.get_attributes.return_value = {"note": "test annotation"}
        loader.store_feature_annotation("f_ann_1", "mRNA_A", "note", "test annotation", doi=None)
