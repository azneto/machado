...
class AssayTest(TestCase):
    ...
    def test_store_assay(self):
        ...
        self.assertEqual(
            True, Db.objects.filter(name=test_assaynamedb).exists()
        )
        test_db = Db.objects.get(name=test_assaynamedb)
        self.assertEqual(
            True,
            Dbxref.objects.filter(
                accession=test_assayacc, db=test_db
            ).exists(),
        )
        ...
        self.assertEqual(
            ...
            Assay.objects.filter(
                name=test_assayname, dbxref=test_dbxref
            ).exists(),
        )
        self.assertEqual(
            True,
            Assayprop.objects.filter(
                assay=test_assay, value=test_filename
            ).exists(),
        )
        ...
        self.assertEqual(
            True, Project.objects.filter(name=test_projectname).exists()
        )
        ...
        self.assertEqual(
            False, AssayProject.objects.filter(assay=test_assay).exists()
        )
