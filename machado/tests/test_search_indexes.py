...
from machado.models import Organism, Feature, FeatureCvterm, FeatureDbxref, Featureprop, Cvterm, Cv, Dbxref, Db
...
class FeatureIndexTest(TestCase):
    def setUp(self):
        ...
        self.dbxref = Dbxref.objects.create(db=self.db, accession="test_acc", version="1")
        ...
        self.type_gene = Cvterm.objects.create(
            name="gene", cv=self.cv_seq, dbxref=self.dbxref,
            is_obsolete=0, is_relationshiptype=0
        )
        self.org = Organism.objects.create(genus="Genus", species="species", common_name="Common")
    ...
    def test_prepare_organism(self):
        ...
        self.assertEqual(self.index.prepare_organism(self.feature), "Genus species infra")
    ...
    def test_prepare_analyses(self, mock_af_filter, mock_fl_filter):
        mock_fl_filter.return_value.filter.return_value.filter.return_value.filter.return_value.values_list.return_value = [1]
        mock_af_filter.return_value.values_list.return_value.distinct.return_value = [('blast',)]
    ...
    def test_prepare_text(self, mock_fr_filter, mock_fc_filter, mock_fd_filter):
        ...
        with patch('machado.search_indexes.Featureloc.objects.filter') as mock_fl_filter:
            mock_loc = MagicMock()
        ...
        self.index.has_overlapping_features = True
        feature_no_rel = MagicMock(spec=['get_display', 'get_annotation', 'get_doi', 'get_expression_samples', 'uniquename', 'name'])
    ...
    @patch('machado.search_indexes.FeatureRelationship.objects.filter')
    def test_prepare_orthologs_biomaterial(self, mock_fr_filter, mock_fp_filter, mock_fp_get):
        mock_fp_get.return_value.value = "OG1"
        mock_fp_filter.return_value.values_list.return_value = [1, 2]
        
        mock_rel = MagicMock()
        mock_rel.subject.get_expression_samples.return_value = [{"biomaterial_description": "bio1"}]
    ...
    @patch('machado.search_indexes.Featureprop.objects.filter')
    @patch('machado.search_indexes.FeatureRelationship.objects.filter')
    def test_prepare_orthologs_coexpression(self, mock_fr_filter, mock_fp_filter, mock_fp_get):
        mock_fp_get.return_value.value = "OG1"
        mock_fp_filter.return_value.values_list.return_value = [1, 2]
        ...
        # have_coexp = True
        with patch('machado.search_indexes.Featureprop.objects.filter') as mock_fp_filter_inner:
    ...
    def test_prepare_autocomplete(self):
        ...
        self.assertIn("Genus species extra keywords", self.index.prepare_autocomplete(self.feature))
        # Test infraspecific name
        self.feature.organism.infraspecific_name = "infra"
        self.assertIn("Genus species infra extra keywords", self.index.prepare_autocomplete(self.feature))
