...
Every data file you'll load (e.g. FASTA, GFF, BLAST) must belong to an organism. These are instructions to load the NCBI taxonomy, that contains most organisms you'll need.
...
If, for any reason, you need to remove a taxonomy, use the commands `remove_phylotree` and `remove_organisms`. Most data files you'll load depend on the organism database (e.g. FASTA, GFF, BLAST). **If you delete an organism database, every data file you loaded will be deleted on cascade.**
